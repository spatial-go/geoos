...
func TestAlgorithm_UnaryUnion(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "UnaryUnion Polygon", args: args{g: space.MultiPolygon{space.Polygon{{{0, 0}, {10, 0}, {10, 10}, {0, 10}, {0, 0}}},
			space.Polygon{{{5, 5}, {15, 5}, {15, 15}, {5, 15}, {5, 5}}}}}, want: []space.Geometry{space.Polygon{{{0, 0}, {10, 0}, {10, 5}, {15, 5}, {15, 15}, {5, 15}, {5, 10}, {0, 10}, {0, 0}}}}, wantErr: false},
		...
		{name: "poly 1",
			...
			want: []space.Geometry{space.MultiPolygon{space.Polygon{{{1, 1}, {1, 2}, {2, 2}, {2, 1}, {1, 1}}},
				space.Polygon{{{3, 1}, {5, 1}, {5, 2}, {3, 2}, {3, 1}}}},
		...
		{name: "poly 2",
			args: args{space.MultiPolygon{space.Polygon{{{1, 1}, {2, 1}, {5, 1}, {5, 2}, {2, 2}, {1, 2}, {1, 1}}},
				space.Polygon{{{2, 1}, {5, 1}, {5, 2}, {2, 2}, {2, 1}}},
			},
			},
			want: []space.Geometry{space.Polygon{{{1, 1}, {5, 1}, {5, 2}, {1, 2}, {1, 1}}},
				space.Polygon{{{1, 1}, {2, 1}, {5, 1}, {5, 2}, {2, 2}, {1, 2}, {1, 1}}},
			},
			wantErr: false},
		...
		{name: "poly 4",
			...
			want: []space.Geometry{space.Polygon{{{1, 2}, {2, 2}, {2, 1}, {5, 1}, {5, 5}, {2, 5}, {2, 3}, {1, 3}, {1, 2}}},
...
func TestAlgorithm_Union(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "UnaryUnion Polygon", args: args{g1: space.Polygon{{{0, 0}, {10, 0}, {10, 10}, {0, 10}, {0, 0}}},
			...
			want:    []space.Geometry{space.Polygon{{{0, 0}, {10, 0}, {10, 5}, {15, 5}, {15, 15}, {5, 15}, {5, 10}, {0, 10}, {0, 0}}}},
			wantErr: false},
			...
			wantErr: false},
		{
			...
			want: []space.Geometry{
				space.Polygon{{{110.85205078125, 38.9252290471405}, {110.72021484375, 37.8054439493427}, {112.124551191479, 37.7177543593912},
					{112.03857421875, 37.3701571840575}, {114.01611328125, 36.2974181865081}, {114.43359375, 37.474858084971},
					{113.994140625, 38.2554363763795}, {113.539811295113, 38.3803991213374}, {113.818359375, 39.0277188402116},
					{112.1484375, 39.5718222373437}, {110.85205078125, 38.9252290471405},
				}},
